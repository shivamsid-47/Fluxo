...
export const dbService = {
    ...
    // Google Login
    loginWithGoogle: async (): Promise<{ user?: UserProfile, isNewUser?: boolean, googleUser?: any, error?: string }> => {
        try {
            ...
            if (userDoc.exists()) {
                ...
                if (userData.blocked) return { error: "Account Blocked." };
                return { user: { ...userData, uid: user.uid } };
            } else {
                // Return 'isNewUser' flag instead of creating immediately
                return { isNewUser: true, googleUser: user };
            }
        } catch (error: any) {
    ...
    // Register Google User (Second Step for New Users)
    registerGoogleUser: async (uid: string, email: string, data: { name: string, phone?: string, role: UserRole }): Promise<{ user?: UserProfile, error?: string }> => {
        try {
            const commonData = {
                uid: uid,
                name: data.name,
            ...
            if (data.role === UserRole.USER) {
                ...
                const newUser: UserProfile = { ...commonData, blocked: false };
                await setDoc(doc(firestore, USERS_COLLECTION, uid), newUser);
                return { user: newUser };

            } else if (data.role === UserRole.INSTITUTION) {
                // Indirect Registration for Organizers (Blocked + Request)
                const newUser: UserProfile = { ...commonData, blocked: true };
                await setDoc(doc(firestore, USERS_COLLECTION, uid), newUser);

                // Create Request
                await addDoc(collection(firestore, REQUESTS_COLLECTION), {
                    name: data.name, // Org Name basically
                    email: email,
                    phone: data.phone || '',
                    uid: uid,
                    status: 'PENDING',
                ...
                return { user: newUser }; // Caller should handle 'blocked' state
            }
